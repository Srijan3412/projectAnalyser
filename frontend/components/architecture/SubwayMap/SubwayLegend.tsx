...
import { Shield, Database, FileText, Activity, AlertTriangle, CheckCircle2, Award, GitMerge } from "lucide-react";
...
export default function SubwayLegend({
  ...
  const rankedFeatures = useMemo(() => {
    ...
    return features.map(f => {
      ...
      const totalReferrers = f.files.reduce((sum: number, fPath: string) => {
        const fileNode = files.find((fn: any) => fn.path === fPath);
    ...
    <div className="flex flex-col bg-zinc-950/40 border border-border/60 rounded-2xl p-4 space-y-4 overflow-hidden h-full select-none">
      ...
      <div className="border-b border-border/50 pb-2.5 shrink-0">
        <h4 className="text-xs font-bold uppercase tracking-widest text-primary flex items-center gap-1.5">
          <GitMerge className="w-3.5 h-3.5 text-primary animate-pulse" />
      ...
      <div className="flex-1 overflow-y-auto space-y-2 pr-1 min-h-0">
        {features.map((feature) => {
          ...
          // Find number of stations on this subway line
          const lineInfo = subway.lines.find(l => l.feature === feature.id);
          const stationCount = lineInfo?.stations.length || 0;

          // Find transfer stations on this line
          const lineTransfers = lineInfo?.stations.filter(sId => subway.transfers.includes(sId)).length || 0;
          ...
          return (
            ...
            >
              ...
              <div className="flex flex-wrap gap-1.5 mt-2.5 pt-2 border-t border-border/20">
                <span className="flex items-center gap-0.5 text-[8.5px] text-zinc-450 font-mono" title="Subway stations">
                  <Activity className="w-2.5 h-2.5 text-zinc-650" />
                  {stationCount} Stations
                </span>
                {lineTransfers > 0 && (
                  <span className="flex items-center gap-0.5 text-[8.5px] text-zinc-450 font-mono" title="Vertical Transfer Interchanges">
                    <GitMerge className="w-2.5 h-2.5 text-zinc-650" />
                    {lineTransfers} Transfers
                  </span>
                )}
                {feature.metrics.tables > 0 && (
                  <span className="flex items-center gap-0.5 text-[8.5px] text-zinc-450 font-mono" title="DB Terminals">
                    <Database className="w-2.5 h-2.5 text-zinc-650" />
